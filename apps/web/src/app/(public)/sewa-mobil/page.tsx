...
import { IconAdjustmentsHorizontal, IconArrowsSort, IconX, IconSearch } from '@tabler/icons-react';
...
function paramsToFilters(params: URLSearchParams): FilterState & { withDriver?: string } {
  return {
    ...
    withDriver: params.get('withDriver') || '', // Local filter for UI only
  };
}

function filtersToParams(f: FilterState & { withDriver?: string }): URLSearchParams {
  ...
  if (f.withDriver) p.set('withDriver', f.withDriver);
  return p;
}
...
function SewaMobilContent() {
  const router = useRouter();
  const searchParams = useSearchParams();

  const [filters, setFilters] = useState<FilterState & { withDriver?: string }>(() => paramsToFilters(searchParams));
  ...
  const fetchVehicles = useCallback(async (f: FilterState & { withDriver?: string }, p: number) => {
    setLoading(true);
    try {
      const [result, activeResult] = await Promise.all([
        getVehicles({
          ...
          withDriver: f.withDriver === 'yes' ? true : f.withDriver === 'no' ? false : undefined,
          page: p,
          limit: PAGE_SIZE,
        }),
        getVehicles({
  ...
  useEffect(() => {
    ...
    router.replace(`/sewa-mobil?${params.toString()}`, { scroll: false });
  }, [filters, page, fetchVehicles, router]);

  const updateFilters = (update: Partial<FilterState & { withDriver?: string }>) => {
  ...
  const handleClearFilters = () => {
    ...
    setPage(1);
    router.replace('/sewa-mobil', { scroll: false });
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      <Breadcrumb items={[{ label: 'Sewa Mobil' }]} />

      <div className="mt-4 mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Sewa Mobil</h1>
          {!loading && (
            <p className="text-sm text-slate-500 mt-0.5">
              {activeTotal.toLocaleString('id-ID')} kendaraan tersedia untuk disewa
            </p>
          )}
        </div>
        ...
        <div className="flex gap-2">
          ...
          <div className="flex items-center gap-2">
            <IconArrowsSort size={14} className="text-slate-400" />
      ...
      {activeFilterCount > 0 && (
        <div className="flex flex-wrap gap-2 mb-4">
          ...
          {filters.withDriver && (
            <span className="flex items-center gap-1 text-xs bg-blue-50 text-blue-700 border border-blue-200 px-2.5 py-1 rounded-full font-medium">
              {filters.withDriver === 'yes' ? 'Dengan Supir' : 'Lepas Kunci'}
              <button onClick={() => updateFilters({ withDriver: '' })}><IconX size={12} /></button>
            </span>
          )}
          <button
      ...
      <div className="flex gap-8">
        ...
        <div className="hidden lg:block w-56 shrink-0">
          <VehicleFilters filters={filters} onChange={updateFilters as (update: Partial<FilterState>) => void} />
          <div className="mt-6 border-t pt-4">
             <p className="font-semibold text-slate-800 mb-2 text-sm">Layanan Supir</p>
             {(['', 'yes', 'no'] as const).map((d) => (
                <label key={d} className="flex items-center gap-2 py-1 cursor-pointer text-sm text-slate-900 hover:text-blue-600 font-medium">
                  <input
                    type="radio"
                    name="withDriver"
                    value={d}
                    checked={filters.withDriver === d}
                    onChange={() => updateFilters({ withDriver: d })}
                    className="accent-blue-600"
                  />
                  {d === '' ? 'Semua' : d === 'yes' ? 'Dengan Supir' : 'Lepas Kunci'}
                </label>
             ))}
          </div>
        </div>
        ...
        <div className="flex-1 min-w-0">
          {!loading && availableVehicles.length === 0 && rentedVehicles.length === 0 && maintenanceVehicles.length === 0 ? (
            <EmptyState
            ...
            <>
              ...
              {!loading && rentedVehicles.length > 0 && (
                <div className="mt-12 mb-6">
                  <div className="relative my-16">
                    ...
                    <div className="relative flex justify-center text-sm uppercase tracking-widest font-semibold">
                      <span className="bg-white px-4 text-gray-500">Sedang Disewa</span>
                    </div>
                  </div>
                  <VehicleGrid
              ...
              {!loading && maintenanceVehicles.length > 0 && (
                <div className="mt-12 mb-6">
                  <div className="relative my-16">
                    <div className="absolute inset-0 flex items-center" aria-hidden="true">
                      <div className="w-full border-t border-gray-200" />
                    ...
                    <div className="relative flex justify-center text-sm uppercase tracking-widest font-semibold">
                      <span className="bg-white px-4 text-gray-500">Kendaraan Dalam Perawatan</span>
                    </div>
                  </div>
                  <VehicleGrid
      ...
      {filtersOpen && (
        <div className="fixed inset-0 z-50 lg:hidden flex flex-col justify-end">
          ...
          <div className="relative bg-white rounded-t-3xl p-5 max-h-[80vh] overflow-y-auto">
            ...
            <VehicleFilters filters={filters} onChange={(u) => { updateFilters(u as Partial<FilterState>); }} />
            <div className="mt-6 border-t pt-4">
             <p className="font-semibold text-slate-800 mb-2 text-sm">Layanan Supir</p>
             {(['', 'yes', 'no'] as const).map((d) => (
                <label key={d} className="flex items-center gap-2 py-1 cursor-pointer text-sm text-slate-900 hover:text-blue-600 font-medium">
