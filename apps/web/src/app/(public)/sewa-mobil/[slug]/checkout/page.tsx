...
import { getVehicleBySlug, submitRentalBooking, getPaymentMethods, uploadLicense, uploadGeneralReceipt } from '@/lib/api';
...
import { IconCircleCheck, IconInfoCircle, IconEye } from '@tabler/icons-react';
...
function CheckoutForm({ vehicle }: { vehicle: Vehicle }) {
  ...
  if (!startStr || !endStr) {
    return <div className="p-8 text-center text-red-500 font-bold">Pilih tanggal terlebih dahulu.</div>;
  }
  ...
  const onSubmit = async (values: CheckoutFormValues) => {
    if (!licenseFile || !receiptFile) return; // Guaranteed by Zod
    setLoading(true);
    try {
      // 1. Upload License & Receipt in parallel
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-xl shadow-slate-200/40 overflow-hidden text-center">
          ...
          <div className="bg-blue-600 p-10 text-white relative overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col items-center">
              ...
              <h2 className="text-3xl font-bold mb-2 tracking-tight">Pemesanan Berhasil!</h2>
              <p className="text-blue-100 font-medium text-lg">Referensi Booking: {successData.bookingCode}</p>
            </div>
          </div>
          ...
          <div className="p-8 lg:p-12 text-left space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div>
                ...
                <div className="space-y-2 text-slate-800">
                  <p className="font-medium text-lg">{successData.customerName}</p>
                  <p>{successData.customerPhone}</p>
                  <p className="text-slate-500">{successData.customerEmail}</p>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-2 text-slate-800">
                  <p className="font-bold text-lg text-slate-900">{vehicle.make} {vehicle.model}</p>
                  <p className="inline-block px-2 py-1 bg-slate-100 text-slate-600 rounded text-sm font-mono mt-1">{vehicle.plateNumber || 'N/A'}</p>
                  <div className="mt-3 text-sm">
                    <p><span className="text-slate-500">Mulai:</span> <span className="font-semibold">{format(new Date(successData.startDate), 'dd MMM yyyy HH:mm', { locale: idLocale })}</span></p>
                    <p><span className="text-slate-500">Selesai:</span> <span className="font-semibold">{format(new Date(successData.endDate), 'dd MMM yyyy HH:mm', { locale: idLocale })}</span></p>
                    <p className="mt-1 text-slate-500">({Math.ceil((new Date(successData.endDate).getTime() - new Date(successData.startDate).getTime()) / (1000 * 60 * 60 * 24))} Hari)</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-6 text-left">
              <div className="flex justify-between items-center border-b border-amber-200 pb-4 mb-4">
                <span className="text-amber-800 font-medium">Total Tagihan (Termasuk Deposit)</span>
                <span className="font-bold text-amber-900 text-2xl">{formatIDR(Number(successData.totalPrice))}</span>
              </div>
              <div>
            ...
            <div className="pt-4 flex justify-center gap-4 border-t border-slate-100">
              <Button onClick={() => router.push('/sewa-mobil')} variant="outline" size="lg" className="w-full sm:w-auto px-8 h-12">Kembali ke Beranda</Button>
            </div>
          </div>
        </div>
      </div>
...
export default function CheckoutPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => {
    async function load() {
      try {
        const v = await getVehicleBySlug(resolvedParams.slug);
        setVehicle(v);
      } catch (err) {
        console.error(err);
      } finally {
    ...
    <div className="min-h-screen bg-slate-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <Breadcrumb items={[
          ...
          { label: vehicle ? `${vehicle.make} ${vehicle.model}` : 'Detail', href: `/sewa-mobil/${resolvedParams.slug}` },
          { label: 'Checkout' }
        ]} />

        <div className="mt-6">
          {loading ? (
            <div className="p-12 text-center text-slate-500 animate-pulse">Memuat data pesanan...</div>
          ) : vehicle ? (
            <Suspense fallback={<div className="p-12 text-center">Memuat form...</div>}>
              <CheckoutForm vehicle={vehicle} />
            </Suspense>
          ) : (
            <div className="p-12 text-center text-red-500">Kendaraan tidak ditemukan.</div>
          )}
        </div>
      </div>
    </div>
